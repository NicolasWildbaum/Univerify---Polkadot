...
function getGenesisConfigCandidates(networkName: string) {
	const candidates = [path.join(CONFIG_DIR, `genesis-${networkName}.json`)];

	if (networkName === "polkadotTestnet") {
		candidates.push(path.join(CONFIG_DIR, "genesis-testnet.json"));
	}
...
function loadGenesisConfig(networkName: string): GenesisConfig {
	const candidates = getGenesisConfigCandidates(networkName);
	const file = candidates.find((candidate) => fs.existsSync(candidate));
	if (!file) {
		throw new Error(
			`Genesis config not found. Checked:\n` +
				candidates.map((candidate) => `  - ${candidate}`).join("\n") +
				`\nCopy contracts/evm/config/genesis-testnet.example.json to ` +
				`contracts/evm/config/genesis-testnet.json and fill in the real issuer set.`,
		);
	}
...
async function main() {
	...
	const viemChain =
		networkName === "polkadotTestnet" ? polkadotHubTestnet : undefined;
